...
*最后更新：2026-09-11 下午（新增 **U40 自动判据里不能掺"人应该多快做完动作"——用独立标尺定位事件时刻**（第一次真跑就因此误判过 FAIL）；**U38 聚合读数不对时先抓一帧原始数据看全局量**；**U39 离线自检通过 ≠ 真机路径可用**（接口真实返回值 / 独占资源生命周期 / 关闭竞态）；另：U33 追加 `.bat` 的 echo 只用 ASCII、U35 追加「可点击路径不能带参数」、U37 双通道信号源）。*
