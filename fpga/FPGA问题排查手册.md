...
| 26 | 资料/图纸 | ★ 原理图 PDF 缺页 → 拿不到芯片球位（先数 `Sheet x of N`） |
...
### 26. ★ 原理图 PDF 缺页 → 拿不到芯片球位（先数 `Sheet x of N`）

**现象**：`高云138K核心板.pdf` 从头翻到尾只有 DDR3、USB-UART、LED/按键、CONNECTOR、POWER，
**找不到 FPGA 符号**，看起来像"图纸没画芯片引出"。

**30 秒定位（两步）**：
1. 数页数 vs 每页图框右下角 `Sheet x of N`。本 PDF 只有 **6 页**而 N=10，页码为 **1 / 6 / 7 / 8 / 9 / 10**
   ⇒ **缺 2、3、4、5 页**。
2. 看 CONNECTOR 页：网络名（`B15_L18_P`、`B34_L24_N`…）左侧带一个小数字，那是**交叉引用页码**；
   凡标 `2` / `3` / `4` 的网络，其"源页"就是缺掉的那几页 —— **芯片本体（含球位）就画在那里**。

**结论**：CONNECTOR 页只到「连接器脚 ↔ 网络名 ↔ 底板功能名」这一层。
`B15_L18_P` 只是画图人起的网络名，**不等于 BGA 球位**（如 `F13`、`R19`）；
球位、专用脚（MODE/READY/DONE）、QSPI、时钟资源全在缺页里，本 PDF 无从补出。

**拿球位三条路**：① 找卖家索要完整 N 页（最快）；② 用工程 `.cst` / `.xdc` 约束反查；
③ 板上量通断 + 芯片 ball map 手推（最慢）。

**附带教训 —— 收资料先核器件名**：该 PDF 封面写 `XC7A100T`（Xilinx Artix-7），
且通篇 bank 编号为 13/14/15/16/34/35，正是 XC7A100T 的六个 IO bank；
而基石板图纸是 `GW5AT-LV60PG484A`，引脚名写作 `IOT93A/LVDS/DQ1`。
两套命名体系完全不同 ⇒ **这份"高云138K"资料本身可能混发/套用了别家模板，拿到先核器件名再往下做**。

**技术细节（解析原理图 PDF）**：Altium 导出的 PDF 常带 `/Rotate 90`，
用 pymupdf 取文字 / 走线坐标要乘 `page.rotation_matrix` 才落到显示坐标系；
图上的网络名分两类——**端口**（压在走线上，y 与走线同高）与**功能名标签**（走线上方约 3.4pt），
只按文字 y 坐标猜归属会串行，**要按连通走线（`get_drawings()` 线段并查集）判定**才准。

---

### 27. ★★★★ "能通讯、但下载/刷写失败" + "接 USB-TTL 点不亮、接可调电源就亮" ⇒ 🔴 先查**供电能力**，不是查芯片/驱动

**现象（本次实测，串口屏 TJC T1 2.0″）**

| # | 现象 |
|---|---|
| **1** | ⭐ **用普通串口终端（VOFA）能连上、能发指令** ⇒ ✅ 链路是通的 |
| **2** | 🔴 **厂商上位机【串口下载工程】失败**，提示"**联机失败**"（⚠️ 握手阶段就挂，根本没开始传） |
| **3** | 🔴 **屏接 USB-TTL 模块时【点不亮】**，⚠️ **但接可调电源就正常点亮** |
| **4** | ⚠️ 关键反证：**同一个 USB-TTL，第一次下载是成功的**（而且是 **921600** 高波特率） |

**根因**

🔴 **USB-TTL 模块的 `5V` 引脚带不动负载** —— 屏的供电（含背光）**电流不够**，
⇒ 电压被拉低到**低于屏的下限**（该屏规格 **4.65~6.5 V**）⇒ 屏**根本没起来**。

⭐ **"点不亮/亮但界面异常"就是最直接的证据**；
⚠️ 而**下载**比**通讯**对供电敏感得多（⭐ 见下方"为什么通讯能过、下载过不去"）。

**为什么"通讯能过、下载过不去"**（⭐ 这是本条的通用形状）

| 维度 | 通讯（发指令） | 🔴 下载 / 刷写 |
|---|---|---|
| **数据量** | 几 ~ 几十字节 | 🔴 **几百 KB ~ 几 MB**（含字库/图片） |
| **持续时间** | 毫秒级 | ⚠️ **几十秒** |
| **对电压跌落** | ⚠️ 容忍度高 | 🔴 **任何一次跌落都可能中断握手或丢数据** |
| **失败表现** | 偶尔丢一帧 | 🔴 **"联机失败" / 中途卡死** |

> ⭐ **官方文档原话**（TJC `QA95`）：
> "以上工具**可以勉强用来通讯**，但是**不要用来下载工程**，
> **通讯和下载的数据量是有几十上百倍的差距**"。
> ⭐ 同一页还点名：**多功能下载器**（带 JLink/STLink、兼 TTL/232/485、多重转换、带隔离）
> **能通讯但下载会挂**；⚠️ **CH340 因假货多、延迟大，也容易"无法下载"**。

⭐⭐ **但本次的真因【不是】**芯片型号 ——
⚠️ **同一块 CH340 第一次下载成功过** ⇒ ⭐ **芯片能力够，是供电不够**。
🔴 **供电不足会让劣质 USB-TTL 表现得更像"芯片/假货问题"**，⭐ **极易误诊**。

**排查优先级（成本从低到高）**

① ⭐⭐ **先看"能不能点亮"** —— 这是**零成本的供电判据**：
   同一负载，**换独立电源能亮 ⇒ 100% 是供电**，⛔ 别再查驱动/芯片/协议；
② ⭐ **把供电和信号【分开】**：外部电源供负载，USB-TTL **只接 TX/RX/GND**；
③ 🔴 **必须共地（GND）** —— ⚠️ 官方 `QA6` 明确要求；
   ⭐ 不共地时**负载照样工作**（它自己有电），🔴 **但串口完全不通** ⇒ ⭐ **这条最容易被漏查**；
④ ⚠️ **量【负载端】的电压**，⛔ 不是看电源显示值（线阻/接触电阻会压降）；
⑤ ⚠️ **降低下载波特率**再试（⭐ 官方：下不来就断电重启后**继续调低**）；
⑥ ⚠️ **缩短接线**（⭐ 官方不建议超过 **30 cm**）；
⑦ ⭐ **极端手段**：点"下载"后**2 秒内给负载断电再上电**（⭐ 官方记录的冷启动同步技巧，
   ⚠️ **只断负载、USB-TTL 必须一直上电**）；
⑧ 最后才考虑**换 CP2102 / FT232 的纯 USB-TTL**（⛔ 不用多功能调试器）。

**教训迁移**

① ⭐⭐ **"能通讯 ≠ 能下载"** —— 二者的**数据量差几十上百倍**，
   ⚠️ **通讯通只证明"链路对"，⛔ 不证明"链路好"**；
② ⭐⭐ **出现"点不亮 / 亮得不对"时，先把供电钉死**，⛔ 不要先怀疑协议、驱动、芯片型号；
③ ⭐⭐⭐ **"第一次成功过"是强判据** —— 它**排除**了"芯片能力不够"这类结论，
   ⭐ 把嫌疑逼到**环境变量**（供电/线材/接触/时机）上（同族 `C28`：先查实际状态）；
④ ⭐ **"接 A 不亮 / 接 B 就亮"是最干净的对照实验** ——
   ⚠️ 只改一个变量（供电来源），⭐ **结论无可争议**；
⑤ 🔴 **不要用多功能调试器去"下载/刷写"** —— ⭐ 官方点名它会挂，
   ⚠️ 而它**通讯是正常的**，🔴 **所以这个坑很隐蔽**。

> 📌 **同族**：本手册 **22**（"物理优先于软件"：插不紧 ⇒ Device not found）、
> **21**（模块插错位 ⇒ RTL 全对也白搭）、
> `共性问题集.md` **C28**（关于外部状态的判断必须现查）、
> `共性问题集.md` **C62**（"无反应"有多个病因，别咬第一个假设）。

---

### 28. ★★★★ `git push` 报 `Permission denied (publickey)` ⇒ 🔴 **先查 `ssh.exe` 在不在 `PATH`**，⛔ 别先怀疑密钥（2026-09-23 实测）
...
⚠️ **这条报错极易误判成**：🔴 密钥失效 / 🔴 权限被改 / 🔴 远端删库 —— **本次三个都不是**。

#### 🎯 根因

```
🔴 `C:\Windows\System32\OpenSSH` **不在 `PATH` 里**
⇒ ⭐ git 拉起的 `ssh` **解析不到可执行文件**
⇒ ⚠️ 被 SSH **报成认证失败**（⭐ 错误信息误导性极强）
```

> ⚠️ 本机 `PATH` 里只有 `C:\Program Files\Git\cmd` 和 `...\Git\bin\bash.exe`，
> ⛔ **没有 OpenSSH 目录**。
...
| # | 动作 | ⭐ 判据 |
|---|---|---|
| **1** | `ssh-add -l` | ⚠️ 若报"**命令找不到**" ⇒ ⭐ **已指向 PATH 问题** |
| **2** | ⭐ **用【完整路径】手测**：<br>`& "C:\Windows\System32\OpenSSH\ssh.exe" -T git@github.com` | ⭐ 若回 `Hi <user>! You've successfully authenticated` ⇒ ⭐ **密钥完全正常，就是 PATH 的事** |
| **3** | `Get-ChildItem $env:USERPROFILE\.ssh` | ⭐ 确认密钥文件在（⭐ 本次 `id_ed25519` 在） |

#### 修法

```powershell
# ⭐ 把 OpenSSH 加进【User PATH】（⚠️ 幂等追加）
$u = [Environment]::GetEnvironmentVariable('PATH','User')
if (($u -split ';') -notcontains 'C:\Windows\System32\OpenSSH') {
    [Environment]::SetEnvironmentVariable(
        'PATH', $u.TrimEnd(';') + ';C:\Windows\System32\OpenSSH', 'User')
}
```
...
```
⭐ 改 PATH 后，**已在运行的进程看不到** ⇒ ⭐ **要重启 DSH Desktop** 才全局生效。
⭐ 重启前应急：⭐ `$env:GIT_SSH = "C:\Windows\System32\OpenSSH\ssh.exe"`
   （⭐ 本次就是这么推成功的；⭐ 重启后确认已自动生效）
```

#### 📌 同族

`共性问题集.md` **C63**（"常见原因"是先验概率，⛔ 不是本次的判据 ——
⭐ 本条里那个"常见原因"就是**"密钥失效"**）、
`C28`（关于外部状态的判断必须现查 —— ⭐ "PATH 里有什么"就是外部状态）、
本手册 **23**（同为"**报错指向 A，真因在 B**"）。
